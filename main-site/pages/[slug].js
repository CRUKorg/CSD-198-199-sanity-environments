...
export async function getStaticPaths() {
  ...
  const allSlugsQuery = groq`*[_type == "page" && site=="mainSite" && section._ref in *[_type=="section" && sectionID=="home"]._id ]{
  _id,
  uid
}`;
...
export async function getStaticProps({ params, preview = false }) {
  console.log(params);
  const query = groq`*[_type == "page" && site=="mainSite" && uid.current == $slug] { title, _id, uid, body}`;
  const queryParams = { slug: `main-site/${params.slug}` };
